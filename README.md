...
It computes:
  1. `P(Z=z|D=d)`, the probability that document `d` is generated from topic `z`
...
where `P(K=k|D=d)` is the probability that keyword `k` generates document `d`, and `nargmax` is the `argmax` for the top `n` keywords. Since `P(Z|D=d)` and `P(K|Z)` are provided by LDA, we can compute `P(K=k|D=d)` easily. All we need to do is select `n` to specify how many keywords we want for each document `d`.
...
![](assets/README-71ce7220.png) regression of mean of top `n` correlations for a particular keyword

![](assets/README-850b0e46.png) for another keyword
...
![](assets/README-e95322b6.png) Can see that most adj R2s plateau at 7-d

![](assets/README-42594eea.png) Steep drop of intercept from 7-d to 8-d
...
![](assets/README-d953d21b.png) For a fixed Keywords-trained model
![](assets/README-cbab9f6a.png) For an LDA-trained model
![](assets/README-9be5aa04.png) Another LDA-trained model
